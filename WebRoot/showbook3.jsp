...
<script type="text/javascript">
	...
	function itembuy(n){
		...
		document.getElementById("buynum"+n).value = buynum;
		location.href = "cartProcess.jsp?action=buy&goodstype=stationery&id="+n+"&buynum="+buynum;
	}
</script>
</head>
<body background="img/bgpic.jpg">
	<center>
		<%
		    ...
		    List list=goodsdao.findtype("文学");
		    session.setAttribute("stationerylist",list);
		%>
		<!-- 用表格来设计主界面 -->
 ...
 %>
					<table border="1" width=100% class="one" bgcolor=#FFFF99>
						...
						<%
							if (stationerylist == null || stationerylist.size() == 0) {
						%>
						<tr>
						...
						<%
								...
								for (int i = 0; i < stationerylist.size(); i++) {
									Goods single = (Goods) stationerylist.get(i);
									int index = i;
						%>
